...
def lowlight_inference(image_path):
...
if __name__ == '__main__':
    # Test the ONNX model on sample images using multi-threading
    filePath = r'data/test_sample/'
    file_list = os.listdir(filePath)

    # Set the number of threads based on the available CPU cores or as needed
    num_threads = min(8, os.cpu_count())
    print(os.cpu_count())

    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        # Use ThreadPoolExecutor to parallelize image processing
        executor.map(lowlight_inference, [os.path.join(filePath, image) for image in file_list])
